  ...
  return (
    <motion.header
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${
        scrolled
          ? 'bg-cream-100/95 backdrop-blur-md shadow-warm border-b border-gold-200/30'
          : 'bg-transparent'
      }`}
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map(({ href, label }) => (
            <Link
              key={label}
              href={href}
              className="font-display text-sm tracking-wide text-choco-700 hover:text-gold-600 relative group"
            >
              {label}
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/login"
            className="hidden md:block px-4 py-2 text-sm font-display tracking-wide text-choco-700 hover:text-gold-600"
          >
            Entrar
          </Link>
          <Link
